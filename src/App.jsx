...
function Dashboard({user,setUser}){
  ...
  // Arka planda sessiz yenileme (30 saniyede bir)
  useEffect(()=>{
  ...
  load();tst("↩ Talep geri alındı, araç ve şoför müsaite çekildi");
        ...
        {tab==="fuel"&&<FuelSection vehicles={vehicles}/>}
...
// ── Yakıt Kayıtları (memo — state korunur) ───────────────────────────────────
const FuelSection=React.memo(function FuelSection({vehicles}){
  ...
  // 30 saniyede bir yenile — ama form açıkken yenileme
  useEffect(()=>{
    const iv=setInterval(()=>{if(!showForm)fetchK();},30000);
    return()=>clearInterval(iv);
  },[fetchK,showForm]);

  const handleCalc=(field,val)=>{
    const upd={...form,[field]:val};
    const lt=parseFloat(field==="lt"?val:upd.lt)||0;
    const fp=parseFloat(field==="birim_fiyat"?val:upd.birim_fiyat)||0;
    upd.toplam_tutar=lt&&fp?(lt*fp).toFixed(2):"";
    setForm(upd);
  };
  ...
  const totTutar=filtered.reduce((a,k)=>a+(parseFloat(k.toplam_tutar)||0),0);
  const totLt=filtered.reduce((a,k)=>a+(parseFloat(k.lt)||0),0);
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center"}}>
        <select value={filPlaka} onChange={e=>setFilPlaka(e.target.value)} style={{padding:"8px 12px",border:"1.5px solid #e2e8f0",borderRadius:8,fontSize:12,fontFamily:"inherit",background:"#fff",outline:"none"}}>
          <option value="">Tüm Araçlar</option>
          {vehicles.map(v=><option key={v.id} value={v.plaka}>{v.plaka}</option>)}
        </select>
        <button onClick={()=>setShowForm(f=>!f)} style={{padding:"9px 18px",borderRadius:9,background:"#0f172a",color:"#fff",border:"none",cursor:"pointer",fontWeight:700,fontSize:13,fontFamily:"inherit"}}>+ Yakıt Ekle</button>
      </div>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12,marginBottom:18}}>
      {[{l:"Toplam Kayıt",v:filtered.length,i:"📋",c:"#3b82f6"},{l:"Toplam Litre",v:`${totLt.toFixed(1)} lt`,i:"⛽",c:"#f59e0b"},{l:"Toplam Tutar",v:`₺${totTutar.toLocaleString("tr-TR",{minimumFractionDigits:2})}`,i:"💰",c:"#ef4444"}].map((s,i)=>(
    ...
    {showForm&&<div style={{background:"#fff",borderRadius:14,padding:20,boxShadow:"0 2px 8px rgba(0,0,0,0.08)",marginBottom:18,border:"1.5px solid #e2e8f0"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:12}}>
        ...
        <div><label style={lbS}>Litre</label><input type="number" step="0.1" placeholder="0.0" value={form.lt} onChange={e=>handleCalc("lt",e.target.value)} style={inpS}/></div>
        <div><label style={lbS}>Birim Fiyat (₺)</label><input type="number" step="0.01" placeholder="0.00" value={form.birim_fiyat} onChange={e=>handleCalc("birim_fiyat",e.target.value)} style={inpS}/></div>
        <div><label style={lbS}>Fatura Numarası</label><input type="text" placeholder="Fatura numarası" value={form.fatura_no} onChange={e=>setForm({...form,fatura_no:e.target.value})} style={inpS}/></div>
        <div><label style={lbS}>Tarih</label><input type="date" value={form.tarih} onChange={e=>setForm({...form,tarih:e.target.value})} style={inpS}/></div>
        <div><label style={lbS}>Firma</label><input type="text" placeholder="Akaryakıt firması" value={form.firma} onChange={e=>setForm({...form,firma:e.target.value})} style={inpS}/></div>
        <div><label style={lbS}>Toplam Tutar (₺)</label><input type="number" step="0.01" placeholder="Otomatik" value={form.toplam_tutar} onChange={e=>setForm({...form,toplam_tutar:e.target.value})} style={{...inpS,background:"#f0fdf4",fontWeight:700,color:"#166534"}}/></div>
      </div>
      <div style={{display:"flex",gap:9,marginTop:14}}>
        ...
        <button onClick={save} disabled={saving} style={{padding:"9px 24px",borderRadius:9,background:"#10b981",color:"#fff",border:"none",cursor:"pointer",fontWeight:700,fontFamily:"inherit"}}>{saving?"Kaydediliyor...":"✓ Kaydet"}</button>
      </div>
    </div>}
    <div style={{background:"#fff",borderRadius:14,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",overflow:"hidden"}}>
      {filtered.length===0
        ...
        :<table className="dt">
          <thead><tr><th>Plaka</th><th>Litre</th><th>Birim Fiyat</th><th>Fatura Numarası</th><th>Tarih</th><th>Firma</th><th>Toplam Tutar</th><th></th></tr></thead>
          <tbody>
            ...
            <tr style={{background:"#f8fafc",fontWeight:700}}>
              <td>TOPLAM</td><td>{totLt.toFixed(1)} lt</td><td colSpan={4}></td>
              <td style={{color:"#ef4444"}}>₺{totTutar.toLocaleString("tr-TR",{minimumFractionDigits:2})}</td><td></td>
            </tr>
          </tbody>
        </table>}
    </div>
